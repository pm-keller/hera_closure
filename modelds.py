...
parser.add_argument(
    "-b", "--bright", help="Path to model visibilities of bright sources.", type=str
)
...
lst_idx = np.where(
    (lstn > np.min(lstm)) & (lstn < np.max(lstm)) & (~np.isnan(sig).all(-1))
)
lstn = lstn[lst_idx]
...
for i in range(shape[1]):
    ...
    for j, triad in enumerate(triads):
        ...
        visn = vis.copy()
        visn.vis = vis.vis + np.array(
            [noise[idx1], noise[idx2], noise[idx3].conjugate()]
        )
